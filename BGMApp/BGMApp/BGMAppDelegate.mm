    ...
    if (@available(macOS 10.14, *)) {
        ...
        dispatch_once(&onceToken, ^{
            ...
            if (micStatus == AVAuthorizationStatusAuthorized) {
                ...
                NSLog(@"BGMAppDelegate::applicationDidFinishLaunching: Microphone permission not granted "
                      "(status=%ld)",
                      (long)micStatus);
                [self showErrorMessage:@"NovaLINK Audio Passthrough needs microphone permission."
                       informativeText:@"It uses a virtual microphone to access your system's "
...
// Sets the "NovaLINK" virtual audio device (BGMDevice) as the user's default audio device.
