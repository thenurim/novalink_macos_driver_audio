...
- (void) insertMenuItemsForApps:(NSArray<NSRunningApplication*>*)apps {
    ...
    try {
        ...
        NSLog(@"BGMAppVolumesController::insertMenuItemsForApps: GetAppVolumes failed '%s' (%d)",
              err4CC,
              err);
    } catch (...) {
        NSLog(@"BGMAppVolumesController::insertMenuItemsForApps: unexpected exception");
    }
}
