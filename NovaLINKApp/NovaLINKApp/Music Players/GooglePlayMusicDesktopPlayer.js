...
window.connect = permanentAuthCode => {
    ...
    window._ws.onmessage = event => {
        // Pass the message along to NovaLINKGooglePlayMusicDesktopPlayerConnection.
    ...
    window._wsPromise = new Promise((resolve, reject) => {
        window._ws.onopen = () => {
            ...
            if (permanentAuthCode) {
                ...
                // telling us to ask the user to type it into NovaLINK.
        ...
        window._ws.onerror = error => {
            // Report the error to NovaLINKGooglePlayMusicDesktopPlayerConnection.
            window.webkit.messageHandlers.error.postMessage(error);
