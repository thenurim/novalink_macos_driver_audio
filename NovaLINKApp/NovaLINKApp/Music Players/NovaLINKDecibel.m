...
@implementation NovaLINKDecibel {
    NovaLINKScriptingBridge* scriptingBridge;
}

- (instancetype) init {
    if ((self = [super initWithMusicPlayerID:[NovaLINKMusicPlayerBase makeID:@"A9790CD5-4886-47C7-9FFC-DD70743CF2BF"]
                                        name:@"Decibel"
...
- (BOOL) pause {
    ...
    if (wasPlaying) {
        DebugMsg("NovaLINKDecibel::pause: Pausing Decibel");
        [self.decibel pause];
    }
...
- (BOOL) unpause {
    ...
    if (wasPaused) {
        DebugMsg("NovaLINKDecibel::unpause: Unpausing Decibel");
        [self.decibel play];
    }
