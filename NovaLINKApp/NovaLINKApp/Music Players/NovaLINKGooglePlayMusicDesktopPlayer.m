...
//
//  NovaLINKGooglePlayMusicDesktopPlayer.m
//  NovaLINKApp
//
...
@implementation NovaLINKGooglePlayMusicDesktopPlayer {
    NovaLINKUserDefaults* userDefaults;
    NovaLINKGooglePlayMusicDesktopPlayerConnection* connection;
...
- (instancetype) initWithUserDefaults:(NovaLINKUserDefaults*)defaults {
    ...
    // command line tool).
    NSUUID* playerID = [NovaLINKMusicPlayerBase makeID:@"FCDCC01F-4BF1-4AD2-BE3E-6B7659A90A3F"];
    if ((self = [super initWithMusicPlayerID:playerID
        ...
        NovaLINKGooglePlayMusicDesktopPlayer* __weak weakSelf = self;

        connection = [[NovaLINKGooglePlayMusicDesktopPlayerConnection alloc]
                initWithUserDefaults:userDefaults
                 authRequiredHandler:^{
                     NovaLINKGooglePlayMusicDesktopPlayer* strongSelf = weakSelf;
        ...
        appWatcher = [[NovaLINKAppWatcher alloc]
                initWithBundleID:NovaLINKNN(self.bundleID)
                     appLaunched:^{
                         NovaLINKGooglePlayMusicDesktopPlayer* strongSelf = weakSelf;
...
- (void) gpmdpWasLaunched {
    if (self.selected) {
        ...
        DebugMsg("NovaLINKGooglePlayMusicDesktopPlayer::gpmdpWasLaunched: GPMDP launched. Connecting");
...
- (void) gpmdpWasTerminated {
    if (self.selected) {
        ...
        DebugMsg("NovaLINKGooglePlayMusicDesktopPlayer::gpmdpWasTerminated: GPMDP has been closed.");
...
- (NSString* __nullable) requestAuthCodeFromUser {
    if (showingAuthCodeDialog) {
        DebugMsg("NovaLINKGooglePlayMusicDesktopPlayer::requestAuthCodeFromUser: "
    ...
    // Ask the user to read the auth code from GPMDP and type it in to NovaLINKApp.
...
- (NSString* __nullable) showAuthCodeDialog {
    // When this isn't being called because the user just changed something in NovaLINKApp (e.g. GPMDP
    // was closed, they selected it in NovaLINKApp for the first time, then opened GPMDP later), we could
    // use notifications instead of an NSAlert. But it probably wouldn't happen often enough to be
    // worth the effort.
    NSAlert* alert = [NSAlert new];
    alert.messageText = @"NovaLINK needs permission to control GPMDP.";
    ...
    if (buttonPressed == NSAlertFirstButtonReturn) {
        ...
        DebugMsg("NovaLINKGooglePlayMusicDesktopPlayer::showAuthCodeDialog: Got auth code: <private>");
        return input.stringValue;
    } else {
        DebugMsg("NovaLINKGooglePlayMusicDesktopPlayer::showAuthCodeDialog: "
                   ...
                   ^{
                       ...
                       NSArray<NSRunningApplication*>* gpmdpApps =
                           [NSRunningApplication
                            runningApplicationsWithBundleIdentifier:NovaLINKNN(self.bundleID)];
...
- (void) showAPIVersionMismatchDialog:(NSString*)reportedAPIVersion {
    ...
    NSString* troubleshootingMsg =
            [NSString stringWithFormat:
             @"GPMDP reported its API version as \"%@\", which NovaLINK doesn't support "
             "yet. NovaLINK might not be able to control GPMDP properly.\n\n"
...
- (void) showErrorDialog:(NSString*)errorMsg troubleshootingMsg:(NSString*)troubleshootingMsg {
    if (!self.running) {
        ...
        DebugMsg("NovaLINKGooglePlayMusicDesktopPlayer::showErrorDialog: Not running");
        return;
    }
...
- (BOOL) isRunning {
    ...
    // callbacks because NovaLINKAutoPauseMenuItem calls this method when it's notified by its own
    // instance of NovaLINKAppWatcher. If NovaLINKAutoPauseMenuItem got notified first, the flag wouldn't be
    // updated in time.
    //
    // At some point we might want to try to avoid this by making the NovaLINKMusicPlayers' running
    // properties observable.
    NSArray<NSRunningApplication*>* instances =
        [NSRunningApplication runningApplicationsWithBundleIdentifier:NovaLINKNN(self.bundleID)];
...
- (BOOL) pause {
    ...
    if (wasPlaying) {
        DebugMsg("NovaLINKGooglePlayMusicDesktopPlayer::pause: Pausing Google Play Music Desktop "
                 "Player");
...
- (BOOL) unpause {
    ...
    if (wasPaused) {
        DebugMsg("NovaLINKGooglePlayMusicDesktopPlayer::unpause: Unpausing Google Play Music Desktop "
                 "Player");
        [connection playPause];
    }
