...
//
//  NovaLINKGooglePlayMusicDesktopPlayerConnection.m
//  NovaLINKApp
//
...
// Self Include
#import "NovaLINKGooglePlayMusicDesktopPlayerConnection.h"
...
@implementation NovaLINKGooglePlayMusicDesktopPlayerConnection {
...
- (instancetype) initWithUserDefaults:(NovaLINKUserDefaults*)defaults
...
- (void) createWebView {
    ...
    NSString* __nullable jsStr =
        (!jsPath ? nil : [NSString stringWithContentsOfFile:NovaLINKNN(jsPath)
    ...
    if (err || !jsStr || [jsStr isEqualToString:@""]) {
        ...
        [self evaluateJavaScript:NovaLINKNN(jsStr)];
    }
}

- (void) connectWithRetries:(int)retries {
    if (retries < 0) {
        NovaLINKAssert(false, "retries < 0");
        return;
    }
    ...
    // NovaLINK uses a bit more memory while connected to GPMDP, around 15 MB for me, but
    // saves us having to complicate the build process to add a dependency on a proper library.
    [self createWebView];

    if (permanentAuthCode) {
        DebugMsg("NovaLINKGooglePlayMusicDesktopPlayerConnection::connectWithRetries: "
                 "Connecting with auth code");

        NSString* __nullable percentEncodedCode =
                [NovaLINKGooglePlayMusicDesktopPlayerConnection
                        toPercentEncoded:NovaLINKNN(permanentAuthCode)];
...
- (void) disconnect {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        DebugMsg("NovaLINKGooglePlayMusicDesktopPlayerConnection::disconnect: Disconnecting");
...
- (void) sendAuthCode:(NSString*)authCode {
    ...
    DebugMsg("NovaLINKGooglePlayMusicDesktopPlayerConnection::sendAuthCode: Sending GPMDP auth code");
    ...
    NSString* __nullable percentEncodedCode =
            [NovaLINKGooglePlayMusicDesktopPlayerConnection toPercentEncoded:authCode];

    // We send the message to GPMDP even if percentEncodedCode is nil so it will reply with an error
    // and NovaLINKApp will ask the user for the auth code again.
...
- (void) sendPermanentAuthCode {
    ...
    if (code) {
        // Don't log the code itself just in case it could be a security problem.
        DebugMsg("NovaLINKGooglePlayMusicDesktopPlayerConnection::sendPermanentAuthCode: "
                 "Sending GPMDP permanent auth code");

        // Percent-encode it just in case something it includes could be executed as Javascript.
        NSString* __nullable percentEncodedCode =
                [NovaLINKGooglePlayMusicDesktopPlayerConnection toPercentEncoded:NovaLINKNN(code)];
        ...
        NSLog(@"NovaLINKGooglePlayMusicDesktopPlayerConnection::sendPermanentAuthCode: No code to send");
    }
}
...
#pragma unused (userContentController)
    ...
    if ([@"log" isEqual:message.name]) {
        ...
        NovaLINKAssert([@"gpmdp" isEqual:message.name], "Unexpected message handler name");
...
- (void) handleLogMessage:(NSString*)message {
    ...
    if (permanentAuthCode) {
        ...
        message = [message stringByReplacingOccurrencesOfString:NovaLINKNN(permanentAuthCode)
                                                     withString:@"<private>"];
    }

    DebugMsg("NovaLINKGooglePlayMusicDesktopPlayerConnection::userContentController: %s",
             message.UTF8String);
#endif
}
...
- (void) handleConnectionError {
    if (connectionRetries > 0) {
        DebugMsg("NovaLINKGooglePlayMusicDesktopPlayerConnection::handleConnectionError: "
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1 * NSEC_PER_SEC)),
                       ...
                       ^{
                           ...
                           if (connectionRetries > 0) {
                               DebugMsg("NovaLINKGooglePlayMusicDesktopPlayerConnection::"
                                        "handleConnectionError: Retrying");
                               [self connectWithRetries:(connectionRetries - 1)];
                           }
                       });
    } else {
        NSLog(@"NovaLINKGooglePlayMusicDesktopPlayerConnection::handleConnectionError: "
...
- (void) handleAPIVersionMessage:(NSDictionary*)body {
    DebugMsg("NovaLINKGooglePlayMusicDesktopPlayerConnection::handleAPIVersionMessage: Response: %s",
    ...
    if (versionParts.count > 0) {
        ...
        DebugMsg("NovaLINKGooglePlayMusicDesktopPlayerConnection::handleAPIVersionMessage: "
...
- (void) handleConnectMessage:(NSDictionary*)body {
    ...
    DebugMsg("NovaLINKGooglePlayMusicDesktopPlayerConnection::handleConnectMessage: Received response");
    ...
    if ([@"CODE_REQUIRED" isEqual:payload]) {
        ...
        if (authCode) {
            [self sendAuthCode:NovaLINKNN(authCode)];
        }
    } else {
...
- (void) handlePlayStateMessage:(NSDictionary*)body {
    (void)body;
    DebugMsg("NovaLINKGooglePlayMusicDesktopPlayerConnection::handlePlayStateMessage: Response: %s",
             [NSString stringWithFormat:@"%@", body].UTF8String);
...
- (void) handleResultMessage:(NSDictionary*)body {
    DebugMsg("NovaLINKGooglePlayMusicDesktopPlayerConnection::handleResultMessage: Response: %s",
             [NSString stringWithFormat:@"%@", body].UTF8String);
