...
//
//  To add support for a music player, create a class that implements the NovaLINKMusicPlayer protocol
//  and add it to initWithAudioDevices in NovaLINKMusicPlayers.mm.
//
//  You'll probably want to subclass NovaLINKMusicPlayerBase and, if the music player supports
//  AppleScript, use NovaLINKScriptingBridge. Your class might need to override the icon method if the
//  default implementation from NovaLINKMusicPlayerBase doesn't work.
//
//  NovaLINKSpotify will probably be the most useful example to follow, but they're all pretty
...
//
//  NovaLINKDriver will use either the music player's bundle ID or PID to match it to the audio it
//  plays. (Though using PIDs hasn't been tested yet.)
//
//  If you're not sure what bundle ID the music player uses, install a debug build of NovaLINKDriver
//  and play something in the music player. The easiest way is to do
//      build_and_install.sh -d
//  NovaLINKDriver will log the bundle ID to system.log when it becomes aware of the music player.
//
...
@protocol NovaLINKMusicPlayer <NSObject>

// Classes return an instance of themselves for each music player app they make available in
// NovaLINKApp. So far that's always been a single instance, but that will probably change eventually.
// Most classes don't need to override the default implementation from NovaLINKMusicPlayerBase.
//
...
//       NovaLINKDevice of changes. It might be easiest to have NovaLINKMusicPlayers to observe this property,
//       on the selected music player, with KVO and update NovaLINKDevice when it changes. Or
//       NovaLINKMusicPlayers could pass a pointer to itself to createInstancesWithDefaults.
...
// need this property and can't just ask NovaLINKDriver if the music player is playing audio).
@property (readonly, getter=isPlaying) BOOL playing;
// True if the music player has a current/open song (or whatever) and will continue playing it if
// NovaLINKMusicPlayer::unpause is called. Normally because the user was playing a song and they or
// NovaLINKApp paused it.
...
@interface NovaLINKMusicPlayerBase : NSObject
...
// NovaLINKMusicPlayer default implementations
+ (NSArray<id<NovaLINKMusicPlayer>>*) createInstancesWithDefaults:(NovaLINKUserDefaults*)userDefaults;
@property (readonly) NSImage* __nullable icon;
@property (readonly) NSUUID* musicPlayerID;
@property (readonly) NSString* name;
