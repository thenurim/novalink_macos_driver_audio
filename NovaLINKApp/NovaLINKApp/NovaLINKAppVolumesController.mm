...
//
//  NovaLINKAppVolumesController.mm
//  NovaLINKApp
//
...
@implementation NovaLINKAppVolumesController {
    // The App Volumes UI.
    NovaLINKAppVolumes* appVolumes;
...
- (id) initWithMenu:(NSMenu*)menu
    ...
    if ((self = [super init])) {
        ...
        appVolumes = [[NovaLINKAppVolumes alloc] initWithController:self
                                                       novaLINKMenu:menu
                                                 appVolumeView:view];
...
- (void) insertMenuItemsForApps:(NSArray<NSRunningApplication*>*)apps {
    ...
    // GetAppVolumes can throw if the virtual device is briefly unavailable (e.g. while NovaLINK
    // Electron occupies/reconfigures audio). This runs from a KVO handler, so uncaught C++
    // exceptions abort the companion and remove the status-bar icon.
    try {
        CACFArray volumesFromNovaLINKDevice([audioDevices novaLINKDevice].GetAppVolumes(), false);

        for (NSRunningApplication* app in apps) {
            if ([self shouldBeIncludedInMenu:app]) {
                NovaLINKAppVolumeAndPan initial = [self getVolumeAndPanForApp:app
                                                             fromVolumes:volumesFromNovaLINKDevice];
        ...
        NSLog(@"NovaLINKAppVolumesController::insertMenuItemsForApps: GetAppVolumes failed '%s' (%d)",
              err4CC,
              err);
    } catch (...) {
        NSLog(@"NovaLINKAppVolumesController::insertMenuItemsForApps: unexpected exception");
    }
}
...
- (NovaLINKAppVolumeAndPan) getVolumeAndPanForApp:(NSRunningApplication*)app
    ...
    NovaLINKAppVolumeAndPan volumeAndPan = {
        .volume = -1,
        .pan = kAppPanNoValue
    };
    ...
    for (UInt32 i = 0; i < volumes.GetNumberItems(); i++) {
        ...
        appVolume.GetCACFString(CFSTR(kNovaLINKAppVolumesKey_BundleID), bundleID);

        pid_t pid;
        appVolume.GetSInt32(CFSTR(kNovaLINKAppVolumesKey_ProcessID), pid);

        if ((app.processIdentifier == pid) ||
            [app.bundleIdentifier isEqualToString:(__bridge NSString*)bundleID.GetCFString()]) {
            // Found a match, so read the volume and pan.
            appVolume.GetSInt32(CFSTR(kNovaLINKAppVolumesKey_RelativeVolume), volumeAndPan.volume);
            appVolume.GetSInt32(CFSTR(kNovaLINKAppVolumesKey_PanPosition), volumeAndPan.pan);
            break;
        }
    }
...
- (BOOL) shouldBeIncludedInMenu:(NSRunningApplication*)app {
    // Ignore hidden apps and NovaLINK itself.
    ...
    BOOL isNovaLINKApp = bundleID && [@kNovaLINKAppBundleID isEqualToString:NovaLINKNN(bundleID)];

    return !isHidden && !isNovaLINKApp;
}
...
forAppWithProcessID:(pid_t)processID
    ...
    audioDevices.novaLINKDevice.SetAppVolume(volume, processID, (__bridge_retained CFStringRef)bundleID);
...
- (void) setAvconferencedVolume:(SInt32)volume {
    ...
    for (int i = 0; i < procCount; i++) {
        ...
        if (proc_pidpath(pid, path, sizeof(path)) > 0 &&
            ...
            audioDevices.novaLINKDevice.SetAppVolume(volume, pid, nullptr);
            return;
        }
    }
...
- (void) setPanPosition:(SInt32)pan
    forAppWithProcessID:(pid_t)processID
               bundleID:(NSString* __nullable)bundleID {
    audioDevices.novaLINKDevice.SetAppPanPosition(pan,
                                             processID,
