...
@implementation NovaLINKAutoPauseMenuItem {
...
- (void) initMenuItemTitle {
    ...
    NovaLINKAutoPauseMenuItem* __weak weakSelf = self;
    ...
    void (^callback)(void) = ^{
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, kMenuItemUpdateWaitTime * NSEC_PER_SEC),
                       ...
                       ^{
                           NovaLINKAutoPauseMenuItem* strongSelf = weakSelf;
                           [strongSelf updateMenuItemTitle];
                       });
    };

    appWatcher = [[NovaLINKAppWatcher alloc] initWithAppLaunched:callback
...
- (void) updateMenuItemTitleWithHighlight:(BOOL)highlight {
    ...
    // running. E.g. someone who only recently installed NovaLINK and doesn't want to use auto-pause at all.
