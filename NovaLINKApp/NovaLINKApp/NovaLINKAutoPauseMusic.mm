...
// bit.
//
// TODO: Would it be worth listening for kAudioDeviceCustomPropertyDeviceIsRunningSomewhereOtherThanNovaLINKApp so we can unpause
...
@implementation NovaLINKAutoPauseMusic {
    BOOL enabled;
    ...
    // True if NovaLINKApp has paused musicPlayer and hasn't unpaused it yet. (Will be out of sync with the music player app if the
    // user has unpaused it themselves.)
    BOOL wePaused;
    // The times, in absolute time, that the NovaLINKDevice last changed its audible state to silent...
    UInt64 wentSilent;
    // ...and to audible.
    UInt64 wentAudible;
}
...
#pragma clang diagnostic pop
        ...
        listenerQueue = dispatch_queue_create("com.bearisdriving.NovaLINK.AutoPauseMusic.Listener", attr);
        pauseUnpauseMusicQueue = dispatch_queue_create("com.bearisdriving.NovaLINK.AutoPauseMusic.PauseUnpauseMusic", attr);
...
- (void) initListenerBlock {
    ...
    __unsafe_unretained NovaLINKAutoPauseMusic* weakSelf = self;
    ...
    listenerBlock = ^(UInt32 inNumberAddresses, const AudioObjectPropertyAddress * _Nonnull inAddresses) {
        ...
        for (int i = 0; i < inNumberAddresses; i++) {
            if (inAddresses[i].mSelector == kAudioDeviceCustomPropertyDeviceAudibleState) {
                NovaLINKDeviceAudibleState audibleState = [weakSelf deviceAudibleState];
                
#if DEBUG
                const char audibleStateStr[5] = CA4CCToCString(audibleState);
                DebugMsg("NovaLINKAutoPauseMusic::initListenerBlock: kAudioDeviceCustomPropertyDeviceAudibleState property changed to '%s'",
                         audibleStateStr);
#endif
                
                // TODO: We shouldn't assume this block will only get called when NovaLINKDevice's audible state changes. (Even if
                //       the Core Audio docs did specify that, there's no reason not to be fault tolerant.)
                if (audibleState == kNovaLINKDeviceIsAudible) {
                    [weakSelf queuePauseBlock];
                } else if (audibleState == kNovaLINKDeviceIsSilent) {
                    [weakSelf queueUnpauseBlock];
                } else if (audibleState == kNovaLINKDeviceIsSilentExceptMusic) {
...
- (NovaLINKDeviceAudibleState) deviceAudibleState {
    return [audioDevices novaLINKDevice].GetAudibleState();
}
...
- (void) queuePauseBlock {
    ...
    DebugMsg("NovaLINKAutoPauseMusic::queuePauseBlock: Dispatching pause block at %llu", now);
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, kPauseDelayNSec),
                   pauseUnpauseMusicQueue,
                   ^{
                       BOOL stillAudible = ([self deviceAudibleState] == kNovaLINKDeviceIsAudible);
                       
                       DebugMsg("NovaLINKAutoPauseMusic::queuePauseBlock: Running pause block dispatched at %llu.%s wentAudible=%llu",
                                startedPauseDelay,
...
- (void) queueUnpauseBlock {
    ...
    UInt64 unpauseDelayNsec =
        static_cast<UInt64>(
            static_cast<Float32>(wentSilent - wentAudible) * kUnpauseDelayWeightingFactor);
    ...
    DebugMsg("NovaLINKAutoPauseMusic::queueUnpauseBlock: Dispatched unpause block at %llu. unpauseDelayNsec=%llu",
             now,
             unpauseDelayNsec);
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, unpauseDelayNsec),
                   pauseUnpauseMusicQueue,
                   ^{
                       BOOL stillSilent = ([self deviceAudibleState] == kNovaLINKDeviceIsSilent);
                       
                       DebugMsg("NovaLINKAutoPauseMusic::queueUnpauseBlock: Running unpause block dispatched at %llu.%s%s wentSilent=%llu",
                                startedUnpauseDelay,
...
- (void) enable {
    if (!enabled) {
        [audioDevices novaLINKDevice].AddPropertyListenerBlock(kNovaLINKAudibleStateAddress, listenerQueue, listenerBlock);
        enabled = YES;
    }
}
...
- (void) disable {
    if (enabled) {
        [audioDevices novaLINKDevice].RemovePropertyListenerBlock(kNovaLINKAudibleStateAddress, listenerQueue, listenerBlock);
        enabled = NO;
    }
}
