...
std::terminate_handler NovaLINKTermination::sOriginalTerminateHandler = std::get_terminate();

CAPThread* const       NovaLINKTermination::sExitSignalsThread = new CAPThread(ExitSignalsProc, nullptr);
sigset_t               NovaLINKTermination::sExitSignals;

NovaLINKAudioDeviceManager* __nullable NovaLINKTermination::sAudioDevices = nullptr;

// If NovaLINKApp crashes, CrashReporter will read this string from our process' memory and include it in
// the crash report.
...
// static
void NovaLINKTermination::TestCrash()
{
    // To give NovaLINKApp a few seconds to finish launching and then crash:
    //
    // dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(2 * NSEC_PER_SEC)),
    //                dispatch_get_main_queue(),
    //                ^{
    //                    NovaLINKTermination::TestCrash();
    //                });
...
// static
void NovaLINKTermination::SetUpTerminationCleanUp(NovaLINKAudioDeviceManager* inAudioDevices)
{
    sAudioDevices = inAudioDevices;

    StartExitSignalsThread();

    // Wrap the default handler for std::terminate, which is called if NovaLINKApp crashes because of an
...
// static
void    NovaLINKTermination::StartExitSignalsThread()
{
...
// static
void    NovaLINKTermination::CleanUpAudioDevices()
{
    // NovaLINKXPCHelper would set the output device back if we didn't do it here, but in general
    // it's better for things to work even if NovaLINKXPCHelper isn't installed.
    if(sAudioDevices)
    {
        [sAudioDevices unsetNovaLINKDeviceAsOSDefault];
    }
}

// static
void    NovaLINKTermination::AddCurrentExceptionToCrashReport()
{
...
// static
void* __nullable NovaLINKTermination::ExitSignalsProc(void* __nullable ignored)
{
    #pragma unused (ignored)

    DebugMsg("NovaLINKTermination::ExitSignalsProc: Thread started.");
