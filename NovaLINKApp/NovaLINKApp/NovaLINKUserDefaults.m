...
@implementation NovaLINKUserDefaults {
...
- (instancetype) initWithDefaults:(NSUserDefaults* __nullable)inDefaults {
    if ((self = [super init])) {
        ...
        // here so we know when it's never been set. (If it hasn't, we try using NovaLINKDevice's
        // kAudioDeviceCustomPropertyMusicPlayerBundleID property to tell which music player should
        // be selected. See NovaLINKMusicPlayers.)
...
- (NovaLINKStatusBarIcon) statusBarIcon {
    NSInteger icon = [self getInt:kDefaultKeyStatusBarIcon or:kNovaLINKStatusBarIconDefaultValue];

    // Just in case we get an invalid value somehow.
    if ((icon < kNovaLINKStatusBarIconMinValue) || (icon > kNovaLINKStatusBarIconMaxValue)) {
        NSLog(@"NovaLINKUserDefaults::statusBarIcon: Unknown NovaLINKStatusBarIcon: %ld", (long)icon);
        icon = kNovaLINKStatusBarIconDefaultValue;
    }

    return (NovaLINKStatusBarIcon)icon;
}

- (void) setStatusBarIcon:(NovaLINKStatusBarIcon)icon {
