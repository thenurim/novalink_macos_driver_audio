...
@implementation NovaLINKXPCListenerDelegate
...
+ (uid_t) _coreaudiodUID {
    ...
    if (sCoreaudiodUID == 0) {
        ...
        if (-1 == bufferSize) {
            @throw @"NovaLINKXPCListenerDelegate::_coreaudiodUID: sysconf failed";
        }
        ...
        if (0 != getpwnam_r("_coreaudiod", &passwd, unusedBuffer, bufferSize, &resultPtr)) {
            @throw @"NovaLINKXPCListenerDelegate::_coreaudiodUID: getpwnam_r failed";
        }
        
        // When getpwnam_r can't find the user it leaves resultPtr null
        if (resultPtr) {
            DebugMsg("NovaLINKXPCListenerDelegate::_coreaudiodUID: Found user: name=%s uid=%u", passwd.pw_name, passwd.pw_uid);
            sCoreaudiodUID = passwd.pw_uid;
        } else {
            DebugMsg("NovaLINKXPCListenerDelegate::_coreaudiodUID: Your system doesn't appear to have a user named \"_coreaudiod\". Unless "
...
- (BOOL) listener:(NSXPCListener*)listener shouldAcceptNewConnection:(NSXPCConnection*)newConnection {
    ...
    DebugMsg("NovaLINKXPCListenerDelegate::listener: Received new connection. effectiveUserIdentifier=%u _coreaudiodUID=%u",
             [newConnection effectiveUserIdentifier],
             [NovaLINKXPCListenerDelegate _coreaudiodUID]);
    
    // Configure the connection.
    // First, set the interface that the exported object implements.
    newConnection.exportedInterface = [NSXPCInterface interfaceWithProtocol:@protocol(NovaLINKXPCHelperXPCProtocol)];
    ...
    newConnection.exportedObject = [[NovaLINKXPCHelperService alloc] initWithConnection:newConnection];
