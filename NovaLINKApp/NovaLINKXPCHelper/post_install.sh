...
#
# Installs NovaLINKXPCHelper's launchd plist file and "bootstraps" (registers/enables) it with launchd.
#
# When you install NovaLINKXPCHelper with xcodebuild (or Xcode itself, if that's possible) this script
...
# making an archive.
#
# TODO: Archiving NovaLINKXPCHelper from Xcode instead of using build_and_install.sh still fails.
...
HELPER_USER=_NovaLINKXPCHelper

LAUNCHD_PLIST_INSTALL_PATH=/Library/LaunchDaemons
LAUNCHD_PLIST_FILENAME=life.thenurim.novalink.XPCHelper.plist
LAUNCHD_PLIST="${LAUNCHD_PLIST_INSTALL_PATH}/${LAUNCHD_PLIST_FILENAME}"

# Create an unprivileged user for NovaLINKXPCHelper to run as.
...
if [[ "$(dscl . -search /Users RecordName ${HELPER_USER})" == "" ]]; then
    ...
    sudo dscl . -create /Groups/${HELPER_USER} RealName "NovaLINK XPC Helper Group"
    ...
    sudo dscl . -create /Users/${HELPER_USER} RealName "NovaLINK XPC Helper"
...
sudo sed -i.tmp "s/{{PATH_TO_NovaLINKXPCHELPER}}/${INSTALL_DIR_ESCAPED}/g" "${LAUNCHD_PLIST}"
# EXECUTABLE_PATH is set by Xcode, currently to "NovaLINKXPCHelper.xpc/Contents/MacOS/NovaLINKXPCHelper".
sudo sed -i.tmp "s/{{NovaLINKXPCHELPER_EXECUTABLE_PATH}}/${EXECUTABLE_PATH_ESCAPED}/g" "${LAUNCHD_PLIST}"
sudo sed -i.tmp "s/{{NovaLINKXPCHELPER_USER_NAME}}/${HELPER_USER}/g" "${LAUNCHD_PLIST}"
sudo sed -i.tmp "s/{{NovaLINKXPCHELPER_GROUP_NAME}}/${HELPER_USER}/g" "${LAUNCHD_PLIST}"
...
# Unregister the plist. This disables the existing version of NovaLINKXPCHelper.
...
# Register the plist with launchd. This enables NovaLINKXPCHelper.
...
echo "Started the NovaLINKXPCHelper service."
