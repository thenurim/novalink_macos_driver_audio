...
#
# Prints the path to a directory the NovaLINKXPCHelper bundle can safely be installed to. Intended to be
# used as the INSTALL_DIR environment variable for xcodebuild commands. For example,
#     xcodebuild -project NovaLINKApp/NovaLINKApp.xcodeproj -target NovaLINKXPCHelper -configuration Debug \
#     DSTROOT="/" INSTALL_DIR="$(NovaLINKApp/NovaLINKXPCHelper/safe_install_dir.sh)" install
#
...
#
# NovaLINKXPCHelper runs as the unprivileged _NovaLINKXPCHelper user, so I don't think it would be a security
# risk right now, but I could be wrong about that. We could also forget about this and give the
# _NovaLINKXPCHelper privileges of some kind in a later version.
#
...
fail() {
    if [[ $ALLOW_UNSAFE_FALLBACK -eq 1 ]]; then
        ...
        echo "$(tput setaf 11)WARNING$(tput sgr0): Installing NovaLINKXPCHelper to its default" \
...
BACKUP_INSTALL_DIR="/Library/Application Support/NovaLINK"
...
if [[ ${DIR_IS_SAFE} -eq 1 ]]; then
    ...
    # Add a "NovaLINK" directory to /Library/Application Support.
