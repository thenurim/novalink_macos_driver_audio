...
pthread_once_t     NovaLINK_NullDevice::sStaticInitializer = PTHREAD_ONCE_INIT;
NovaLINK_NullDevice*    NovaLINK_NullDevice::sInstance = nullptr;

NovaLINK_NullDevice&    NovaLINK_NullDevice::GetInstance()
{
    pthread_once(&sStaticInitializer, StaticInitializer);
    return *sInstance;
}

void    NovaLINK_NullDevice::StaticInitializer()
{
    try
    {
        sInstance = new NovaLINK_NullDevice;
        // Note that we leave the device inactive initially. NovaLINKApp will activate it when needed.
    }
    catch(...)
    {
        DebugMsg("NovaLINK_NullDevice::StaticInitializer: Failed to create the device");
        delete sInstance;
        sInstance = nullptr;
    }
}

NovaLINK_NullDevice::NovaLINK_NullDevice()
:
    NovaLINK_AbstractDevice(kObjectID_Device_Null, kAudioObjectPlugInObject),
...
void    NovaLINK_NullDevice::Activate()
{
...
void    NovaLINK_NullDevice::Deactivate()
{
    CAMutex::Locker theStateLocker(mStateMutex);
...
void    NovaLINK_NullDevice::SendDeviceIsAlivePropertyNotifications()
{
    CADispatchQueue::GetGlobalSerialQueue().Dispatch(false, ^{
        ...
        NovaLINK_PlugIn::Host_PropertiesChanged(GetObjectID(), 1, theChangedProperties);
    });
}

#pragma mark Property Operations

bool    NovaLINK_NullDevice::HasProperty(AudioObjectID inObjectID,
                                    pid_t inClientPID,
...
bool    NovaLINK_NullDevice::IsPropertySettable(AudioObjectID inObjectID,
                                           pid_t inClientPID,
                                           const AudioObjectPropertyAddress& inAddress) const
{
...
UInt32    NovaLINK_NullDevice::GetPropertyDataSize(AudioObjectID inObjectID,
                                            pid_t inClientPID,
                                            ...
                                            const void* __nullable inQualifierData) const
{
    ...
    switch(inAddress.mSelector)
    {
        ...
        default:
            theAnswer = NovaLINK_AbstractDevice::GetPropertyDataSize(inObjectID,
                                                                inClientPID,
                                                                inAddress,
                                                                inQualifierDataSize,
                                                                inQualifierData);
...
void    NovaLINK_NullDevice::GetPropertyData(AudioObjectID inObjectID,
                                        pid_t inClientPID,
                                        const AudioObjectPropertyAddress& inAddress,
                                        UInt32 inQualifierDataSize,
                                        ...
                                        void* outData) const
{
    ...
    // See NovaLINK_Device::Device_GetPropertyData for more information about these properties.
    ...
    switch(inAddress.mSelector)
    {
        case kAudioObjectPropertyName:
            ThrowIf(inDataSize < sizeof(AudioObjectID),
                    ...
                    "NovaLINK_NullDevice::GetPropertyData: not enough space for the return value of "
        ...
        case kAudioDevicePropertyDeviceUID:
            ...
            *reinterpret_cast<CFStringRef*>(outData) = CFSTR(kNovaLINKNullDeviceUID);
            outDataSize = sizeof(CFStringRef);
            break;
        ...
        case kAudioDevicePropertyModelUID:
            ...
            *reinterpret_cast<CFStringRef*>(outData) = CFSTR(kNovaLINKNullDeviceModelUID);
            outDataSize = sizeof(CFStringRef);
            break;
        ...
        default:
            NovaLINK_AbstractDevice::GetPropertyData(inObjectID,
                                                inClientPID,
                                                inAddress,
                                                inQualifierDataSize,
                                                inQualifierData,
...
void    NovaLINK_NullDevice::SetPropertyData(AudioObjectID inObjectID,
                                        pid_t inClientPID,
                                        const AudioObjectPropertyAddress& inAddress,
                                        UInt32 inQualifierDataSize,
                                        ...
                                        const void* inData)
{
    ...
    {
        NovaLINK_AbstractDevice::SetPropertyData(inObjectID,
                                            inClientPID,
                                            inAddress,
                                            inQualifierDataSize,
                                            inQualifierData,
...
void    NovaLINK_NullDevice::StartIO(UInt32 inClientID)
{
    ...
    if(mClientsDoingIO == 0)
    {
        ...
        // Send notifications.
        DebugMsg("NovaLINK_NullDevice::StartIO: Sending kAudioDevicePropertyDeviceIsRunning");
        CADispatchQueue::GetGlobalSerialQueue().Dispatch(false, ^{
            AudioObjectPropertyAddress theChangedProperty[] = {
                CAPropertyAddress(kAudioDevicePropertyDeviceIsRunning)
            };
            NovaLINK_PlugIn::Host_PropertiesChanged(kObjectID_Device_Null, 1, theChangedProperty);
        });
    }
...
void    NovaLINK_NullDevice::StopIO(UInt32 inClientID)
{
    #pragma unused (inClientID)
    ...
    ThrowIf(mClientsDoingIO == 0,
            ...
            "NovaLINK_NullDevice::StopIO: Underflowed mClientsDoingIO");
    ...
    {
        // Send notifications.
        DebugMsg("NovaLINK_NullDevice::StopIO: Sending kAudioDevicePropertyDeviceIsRunning");
        CADispatchQueue::GetGlobalSerialQueue().Dispatch(false, ^{
            AudioObjectPropertyAddress theChangedProperty[] = {
                CAPropertyAddress(kAudioDevicePropertyDeviceIsRunning)
            };
...
void    NovaLINK_NullDevice::GetZeroTimeStamp(Float64& outSampleTime,
                                         UInt64& outHostTime,
                                         UInt64& outSeed)
{
    CAMutex::Locker theIOLocker(mIOMutex);
    ...
    // clock similar to the loopback clock in NovaLINK_Device.
    ...
    outSampleTime = static_cast<Float64>(mNumberTimeStamps) * static_cast<Float64>(kZeroTimeStampPeriod);
    outHostTime = static_cast<UInt64>(static_cast<Float64>(mAnchorHostTime) + theHostTicksSinceAnchor);
    outSeed = 1;
}

void    NovaLINK_NullDevice::WillDoIOOperation(UInt32 inOperationID,
                                          bool& outWillDo,
...
void    NovaLINK_NullDevice::DoIOOperation(AudioObjectID inStreamObjectID,
                                      UInt32 inClientID,
