...
#ifndef __NovaLINKDriver__NovaLINK_PlugIn__
#define __NovaLINKDriver__NovaLINK_PlugIn__
...
public:
    static NovaLINK_PlugIn&				GetInstance();

protected:
                                    NovaLINK_PlugIn();
	virtual							~NovaLINK_PlugIn();
...
public:
    const CFStringRef               GetBundleID() const { return CFSTR(kNovaLINKDriverBundleID); }
    
private:
    CAMutex							mMutex;
    
    static pthread_once_t			sStaticInitializer;
    static NovaLINK_PlugIn*				sInstance;
	static AudioServerPlugInHostRef	sHost;

};

#endif /* __NovaLINKDriver__NovaLINK_PlugIn__ */
