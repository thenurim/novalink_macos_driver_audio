...
//
//  An input or output audio stream. Each stream belongs to a device (see NovaLINK_AbstractDevice), which
//  in turn belongs to a plug-in (see NovaLINK_PlugIn).
//
//  This class only handles the stream's HAL properties, i.e. the metadata about the stream, not the
//  audio data itself.
//

#ifndef NovaLINKDriver__NovaLINK_Stream
#define NovaLINKDriver__NovaLINK_Stream
...
public:
                                NovaLINK_Stream(AudioObjectID inObjectID,
                                           AudioObjectID inOwnerDeviceID,
                                           bool inIsInput,
                                           Float64 inSampleRate,
                                           UInt32 inStartingChannel = 1);
    virtual                     ~NovaLINK_Stream();
...
#endif /* NovaLINKDriver__NovaLINK_Stream */
