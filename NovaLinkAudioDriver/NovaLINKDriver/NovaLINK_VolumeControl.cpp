...
NovaLINK_VolumeControl::NovaLINK_VolumeControl(AudioObjectID inObjectID,
...
bool    NovaLINK_VolumeControl::HasProperty(AudioObjectID inObjectID,
                                       pid_t inClientPID,
...
bool    NovaLINK_VolumeControl::IsPropertySettable(AudioObjectID inObjectID,
                                              pid_t inClientPID,
                                              const AudioObjectPropertyAddress& inAddress) const
{
    CheckObjectID(inObjectID);
...
UInt32  NovaLINK_VolumeControl::GetPropertyDataSize(AudioObjectID inObjectID,
                                               pid_t inClientPID,
...
void    NovaLINK_VolumeControl::GetPropertyData(AudioObjectID inObjectID,
                                           pid_t inClientPID,
                                           const AudioObjectPropertyAddress& inAddress,
                                           UInt32 inQualifierDataSize,
    ...
    switch(inAddress.mSelector)
    {
        case kAudioLevelControlPropertyScalarValue:
            // This returns the value of the control in the normalized range of 0 to 1.
            {
                ThrowIf(inDataSize < sizeof(Float32),
                        ...
                        "NovaLINK_VolumeControl::GetPropertyData: not enough space for the return value "
        ...
        case kAudioLevelControlPropertyDecibelRange:
            ...
            ThrowIf(inDataSize < sizeof(AudioValueRange),
                    CAException(kAudioHardwareBadPropertySizeError),
                    "NovaLINK_VolumeControl::GetPropertyData: not enough space for the return value of "
...
void    NovaLINK_VolumeControl::SetPropertyData(AudioObjectID inObjectID,
                                           pid_t inClientPID,
                                           const AudioObjectPropertyAddress& inAddress,
                                           UInt32 inQualifierDataSize,
                                           const void* inQualifierData,
        ...
        case kAudioLevelControlPropertyScalarValue:
            {
                ThrowIf(inDataSize != sizeof(Float32),
                        CAException(kAudioHardwareBadPropertySizeError),
                        "NovaLINK_VolumeControl::SetPropertyData: wrong size for the data for "
...
void    NovaLINK_VolumeControl::SetVolumeScalar(Float32 inNewVolumeScalar)
{
...
void    NovaLINK_VolumeControl::SetVolumeDb(Float32 inNewVolumeDb)
{
...
void    NovaLINK_VolumeControl::SetWillApplyVolumeToAudio(bool inWillApplyVolumeToAudio)
{
...
bool    NovaLINK_VolumeControl::WillApplyVolumeToAudioRT() const
{
    return mWillApplyVolumeToAudio;
}

void    NovaLINK_VolumeControl::ApplyVolumeToAudioRT(Float32* ioBuffer, UInt32 inBufferFrameSize) const
{
    ThrowIf(!mWillApplyVolumeToAudio,
            CAException(kAudioHardwareIllegalOperationError),
            "NovaLINK_VolumeControl::ApplyVolumeToAudioRT: This control doesn't process audio data");
...
void    NovaLINK_VolumeControl::SetVolumeRaw(SInt32 inNewVolumeRaw)
{
    CAMutex::Locker theLocker(mMutex);
    ...
    if(mVolumeRaw != inNewVolumeRaw)
    {
        ...
        NovaLINKAssert((mAmplitudeGain >= 0.0f) && (mAmplitudeGain <= 1.0f), "Gain not in [0,1]");
        ...
        CADispatchQueue::GetGlobalSerialQueue().Dispatch(false, ^{
            ...
            NovaLINK_PlugIn::Host_PropertiesChanged(GetObjectID(), 2, theChangedProperties);
        });
    }
}
