...
#
# If NovaLINKDevice is set as your default device when you run this, you might have to change your
# default device and change it back to NovaLINKDevice to get programs that use audio working again.
...
# This config file just stores the paths to your NovaLINKDriver builds (at least so far)
...
get_build_path() {
    if [[ ${USE_RELEASE_BUILD} == true ]] && [[ "${DRIVER_PATH_RELEASE}" != "" ]] && [[ -d "${DRIVER_PATH_RELEASE}" ]]; then
        ...
        echo -n "Enter the absolute path to your NovaLINKDriver"
        ([[ ${USE_RELEASE_BUILD} == true ]] && echo -n " release ") || echo -n " debug "
        echo "build. Probably something like"
        echo -n "/Users/$(whoami)/Library/Developer/Xcode/DerivedData/NovaLINK-somerandomchars/Build/Products/"
        ([[ ${USE_RELEASE_BUILD} == true ]] && echo -n "Release") || echo -n "Debug"
        echo "/NovaLINK Audio Device.driver"
        cd /
...
INSTALLED_DRIVER_PATH="${HAL_PLUGINS_DIR}/NovaLINK Audio Device.driver"
...
if [[ ${DONT_RESTART_COREAUDIOD} == false ]]; then
    echo "$(bold_face Restarting coreaudiod). If a running app stops playing audio, change your default audio device (and change it back if you want) or open NovaLINKApp."
