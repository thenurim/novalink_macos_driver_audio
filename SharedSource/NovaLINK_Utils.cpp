...
dispatch_queue_t NovaLINKGetDispatchQueue_PriorityUserInteractive()
{
    long queueClass;
                                            ...
                                            const std::function<void(void)>& function)
    {
        ...
        catch(const CAException& e)
        {
            ...
            logFn("%s:%d:%s: %sCAException, code: '%s' (%d). %s%s %s %s ",
                     ...
                     kNovaLINKIssueTrackerURL);
            
#if NovaLINK_StopDebuggerOnLoggedExceptions || NovaLINK_StopDebuggerOnLoggedUnexpectedExceptions
#if !NovaLINK_StopDebuggerOnLoggedExceptions
            if(!expected)
#endif
            {
                NovaLINKAssert(false, "CAException");
            }
#endif
            return e.GetError();
        }
...
#if NovaLINK_StopDebuggerOnLoggedExceptions || NovaLINK_StopDebuggerOnLoggedUnexpectedExceptions
            NovaLINKAssert(false, "Unknown exception");
#endif
            return -1;
        }
