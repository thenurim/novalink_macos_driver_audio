...
if [[ $? -eq 0 ]] && ( [[ "${TAG}" == "" ]] || \
    ...
    if [[ -f "$info_plist" ]]; then
        ...
        base_version=$(/usr/libexec/PlistBuddy -c "Print :NovaLINKBundleVersionBase" "${info_plist}" 2>/dev/null)

        if [[ $? -ne 0 ]] || [[ "${base_version}" == "" ]]; then
            base_version="${current_version}"
            /usr/libexec/PlistBuddy -c "Add :NovaLINKBundleVersionBase string ${base_version}" "${info_plist}"
        fi
